...
<template>
  <n-layout :native-scrollbar="false" :has-sider="true" class="layout">
    <n-el class="gallary-layout" :class="settings.darkTheme || settings.darkSider ? 'dark' : 'light'">
      <!-- 侧边栏 也就是一级目录 -->
      <div :style="{
        ...
        <div class="content">
          <n-scrollbar>
            <div v-for="route in firstLevelMenus" :key="route.path" @click="onMenuClick(route)" class="sider-item"
              ...
              <!-- 图标下的文本，超长后截断 -->
              <n-ellipsis v-if="settings.siderCollapsed || settings.siderCollapsedWidth > data.siderOpenWidth - 10"
                :style="{ maxWidth: data.siderOpenWidth - 10 + 'px' }">{{ $t("route." + route.name) }}</n-ellipsis>
            </div>
          </n-scrollbar>
        </div>
        <!-- 底部收缩按钮 -->
        <div class="footer-icon">
          <n-tooltip placement="right-start" trigger="hover">
            <template #trigger>
              <g-icon @click="settings.setSiderCollapsed(!settings.getSiderCollapsed)" :size="18"
                :icon="settings.siderCollapsed ? SysIcons.collpsedLeft : SysIcons.expandRight"></g-icon>
            </template>
      ...
      <!-- 目录菜单，也就是二级和以下的 Menu -->
      <div :style="{ width: data.isFixed ? settings.siderWidth + 'px' : 0 }" class="menu-wrapper duration ease-in-out">
        <div style="z-index: 10" @mouseleave="data.isShowMenu = data.isFixed || false"
          ...
          <header>
            ...
            <span @click.stop="data.isFixed = !data.isFixed" class="pin-icon">
              <n-tooltip trigger="hover">
                <template #trigger>
                  <g-icon :color="settings.darkSider || settings.darkTheme ? 'white' : 'black'"
                    :icon="data.isFixed ? SysIcons.pin : SysIcons.pinOff"></g-icon>
                </template>
    ...
    <n-layout-content :native-scrollbar="false">
      <n-layout :native-scrollbar="false" position="absolute">
        ...
        <n-layout position="absolute" class="header">
          ...
          <n-layout-footer v-show="settings.showFooter" :inverted="settings.darkFooter" bordered position="absolute">
            <g-footer></g-footer>
          </n-layout-footer>
        </n-layout>
      </n-layout>
